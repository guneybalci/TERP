...
!(function (a) {
  ...
  function b(h, i, k, m) {
    ...
    function p() {
      A.data(i, C)
        .addClass(z.input)
        .val(A.data("value") ? C.get("select", y.format) : h.value)
        .on(
          "focus." + x.id + " click." + x.id,
          f(function (a) {
        ...
        g(h, { haspopup: !0, expanded: !1, readonly: !1, owns: h.id + "_root" });
    }
    function q() {
      g(C.$root[0], "hidden", !0);
    }
      ...
      C.$holder
        ...
        .on("click", "[data-pick], [data-nav], [data-clear], [data-close]", function () {
          ...
          (e = e && (e.type || e.href ? e : null)),
            ...
            !d && c.nav ? C.set("highlight", C.component.item.highlight, { nav: c.nav }) : !d && "pick" in c ? (C.set("select", c.pick), y.closeOnSelect && C.close(!0)) : c.clear ? (C.clear(), y.closeOnClear && C.close(!0)) : c.close && C.close(!0);
        });
    }
    function s() {
      var b;
      !0 === y.hiddenName ? ((b = h.name), (h.name = "")) : ((b = ["string" == typeof y.hiddenPrefix ? y.hiddenPrefix : "", "string" == typeof y.hiddenSuffix ? y.hiddenSuffix : "_submit"]), (b = b[0] + h.name + b[1])),
    ...
    function t() {
      w && n
        ? C.$holder.find("." + z.frame).one("transitionend", function () {
            C.$holder.eq(0).focus();
          })
        : setTimeout(function () {
            C.$holder.eq(0).focus();
    ...
    if (!h) return b;
    var w = !1,
      x = { id: h.id || "P" + Math.abs(~~(Math.random() * new Date())), handlingOpen: !1 },
        ...
        start: function () {
          return x && x.start
            ...
            : ((x.methods = {}),
              ...
              C.on({ start: C.component.onStart, render: C.component.onRender, stop: C.component.onStop, open: C.component.onOpen, close: C.component.onClose, set: C.component.onSet }).on({
                start: y.onStart,
                render: y.onRender,
                stop: y.onStop,
                open: y.onOpen,
          ...
          return x.open
            ...
            : (A.addClass(z.active),
              ...
              !1 !== c &&
                ...
                t(),
                l
                  .on("click." + x.id + " focusin." + x.id, function (a) {
                    if (!x.handlingOpen) {
                      var b = e(a, h);
                  ...
                  .on("keydown." + x.id, function (c) {
                    ...
                    27 == d
                      ? C.close(!0)
                      : g != C.$holder[0] || (!f && 13 != d)
                      ? a.contains(C.$root[0], g) && 13 == d && (c.preventDefault(), g.click())
                      : (c.preventDefault(), f ? b._.trigger(C.component.key.go, C, [b._.trigger(f)]) : C.$root.find("." + z.highlighted).hasClass(z.disabled) || (C.set("select", C.component.item.highlight), y.closeOnSelect && C.close(!0)));
                  })),
              C.trigger("open"));
        },
        close: function (b) {
  ...
  return (
    (b.klasses = function (a) {
      return (
        (a = a || "picker"),
        { picker: a, opened: a + "--opened", focused: a + "--focused", input: a + "__input", active: a + "__input--active", target: a + "__input--target", holder: a + "__holder", frame: a + "__frame", wrap: a + "__wrap", box: a + "__box" }
      );
    }),
    (b._ = {
      group: function (a) {
