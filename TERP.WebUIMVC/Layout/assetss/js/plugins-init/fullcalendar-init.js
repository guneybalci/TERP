! function (e) {
    ...
    t.prototype.onDrop = function (t, n) {
        ...
        var i = e("<form></form>");
        i.append("<div class='row'></div>"), i.find(".row").append("<div class='col-md-6'><div class='form-group'><label class='control-label'>Etkinlik Adı</label><input class='form-control trenchancy' placeholder='Etkinliğinizi Girin' type='text' name='title'/></div></div>").append("<div class='col-md-6'><div class='form-group'><label class='control-label'>Kategori</label><select class='form-control trenchancy' name='category'></select></div></div>").find("select[name='category']").append("<option value='bg-danger'>Önemli</option>").append("<option value='bg-success'>Başarılı</option>").append("<option value='bg-dark'>Siyah</option>").append("<option value='bg-primary'>Birincil</option>").append("<option value='bg-pink'>Pembe</option>").append("<option value='bg-info'>Bilgi</option>").append("<option value='bg-warning'>Uyarı</option></div></div>"), o.$modal.find(".delete-event").hide().end().find(".save-event").show().end().find(".modal-body").empty().prepend(i).end().find(".save-event").unbind("click").on("click", function () {
            i.submit()
        }), o.$modal.find("form").on("submit", function () {
